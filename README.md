...
- JWT-аутентификация и ролевой доступ (ADMIN / USER)
- Жизненный цикл карт: `ACTIVE` → `BLOCK_REQUESTED` → `BLOCKED` / `EXPIRED`
- Шифрование номеров карт (AES/GCM) и маскирование в ответах API
- Транзакционные переводы только между своими картами
- Заявки на блокировку с подтверждением/отклонением администратором
- Фильтрация и пагинация карт (JPA Specifications)
- Миграции схемы (Liquibase), документация OpenAPI/Swagger
...
Перед запуском создай `.env` в корне проекта (значения ниже — демонстрационные; для продакшена задай свои):
